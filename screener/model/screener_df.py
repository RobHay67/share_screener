			...
			if scope.pages[page]['refresh_ticker_df'][ticker] == True:										# so we only refresh if we have been asked to
				if ticker in scope.ticker_data_files:														# if its not in here, it will not be available
					...
					if analysis_row_limit != None : 
						screener_df = screener_df.head(analysis_row_limit) 									# limit analysis to user specified row limit
					...
					print ( '\033[91m' + ticker + ' < create_screener_df > Need to Check all test results for an overall filter somewhere - here??' + '\033[0m')
