...
from users.scope.save.trials_config import save_trials_user_settings
from users.scope.save.charts_config import save_charts_user_settings
...
def save_users_table(scope):
	...
	if user != 'Login to Use the Application':
		...
		scope.users['json'][user]['external_link'] = scope.config['external_link']
