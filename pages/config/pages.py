...
from pages.config.ticker_data import render_ticker_df, render_ticker_page_config
...
def render_page_config(scope):
	...
	st.divider()
	st.header('Settings for '+page+' page.')
	
	if page == 'screener':render_trials_config(scope)
	if page == 'chart':render_charts_config(scope)
