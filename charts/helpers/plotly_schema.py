...
def create_schema_for_plotly(scope):
	logging.warning("create_schema_for_plotly")
	schema = {
			'no_of_charts'			: 0,
			'col_no' 				: 1,
			'chart_heights' 		: [],
	...
	for chart in scope.charts['chart_list']:	
		if scope.charts['user_config'][chart]['active'] == True and scope.charts['user_config'][chart]['is_overlay'] == False: 			# Charts - Selected (active) and IS NOT an overlay
			schema['no_of_charts'] = schema['no_of_charts'] + 1
			schema['chart_heights'].append(scope.charts['user_config'][chart]['plot']['scale'])
			schema['add_chart'].append(chart)
		elif  scope.charts['user_config'][chart]['active'] == True and scope.charts['user_config'][chart]['is_overlay'] == True:			# Overlays - Selected (active) and IS an overlay
...
def make_chart_heights_proportional(scope, schema):
	logging.warning("make_chart_heights_proportional")
	...
	scope.charts['total_height'] = sum(schema['chart_heights'])
	return schema
