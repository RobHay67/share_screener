import streamlit as st

# blue, green, orange, red, violet, gray/grey, rainbow.

def data_type_button(scope, button_type, number=1):
	widget_key = 'widget>'+button_type+str(number)
	match button_type:
		case 'blank'		:button_label = ' '
		case 'dict'			:button_label=":rainbow[{ }]"
		case 'list'			:button_label=":orange[[ list ]]"
		case 'string'		:button_label=":violet['string']"
		case 'integer'		:button_label=":blue[integer]"
		case 'true_false'	:button_label=":green[True] or :red[False]"
		case 'password'		:button_label=":red[*********]"
		case _				:button_label=":red[ERROR]"

	button = st.button(
		label=button_label, 
		key=widget_key, 
		use_container_width=True, 
		type='secondary',
		)
	return button
