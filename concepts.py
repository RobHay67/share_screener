





# Application Control

#							ok			this is good			this is good					this makes sense - we just check in here as needed
# ========================================================================================================================================================================
#  type of control			widget		populated_from			selection_stored_in				loaded_/_download_data_stored_in	Page Dataframe
# ------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Company Profile			selectbox	dropdown_ticker		 	ticker_for_company_profile		share_data_files[ticker]			share_data_for_company_profile[ticker]
# Volume Predictor			selectbox	dropdown_ticker			ticker_for_vol_predict			share_data_files[ticker]
# Daily Analysis			selectbox	??						??								share_data_files[ticker]
# Multi Ticker Analysis		multiselect	dropdown_markets		tickers_for_multi				share_data_files[ticker]
# Multi Ticker Analysis		multiselect	dropdown_industries		tickers_for_multi				share_data_files[ticker]
# Multi Ticker Analysis		multiselect	dropdown_tickers		tickers_for_multi				share_data_files[ticker]
